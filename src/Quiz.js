...
const Quiz = () => {
  ...
  const questions = [
    ...
    { text: '건물 관리 회의가 잡혔다.첫 회의를 위해,', options: [{ text: '굳이 참석 해야하나..?', type: 'I' }, { text: '적극적으로 의견 어필해서분위기를 휘어잡을거야!', type: 'E' }] },
    ...
    { text: '친구가 돈을 빌려달라고 부탁했다.\n당신의 반응은?', options: [{ text: '왜 필요해? 얼마나 필요해? 언제 갚을 수 있어?', type: 'T' }, { text: '요즘 힘드냐? 이럴 때 도와주는게 친구지 얼마면 되!', type: 'F' }] },
  ...
  return (
    <div className="h-screen flex items-center justify-center">
      {shuffledQuestions[currentQuestion] ? (
        <div className="flex flex-col justify-between h-full max-w-md w-full bg-gray-100 py-10 px-5 font-title">
          {/* 상단 질문 영역 */}
          ...
          {/* 하단 버튼 영역 */}
          <div className="grid gap-4 mt-4 text-md">
            {shuffledQuestions[currentQuestion].options.map((option, index) => (
              <button key={index} onClick={() => handleAnswerClick(option.type)} style={{ whiteSpace: 'pre-wrap' }} className="py-2 px-4 rounded whitespace-pre-line">
                {option.text}
              </button>
            ))}
          </div>
